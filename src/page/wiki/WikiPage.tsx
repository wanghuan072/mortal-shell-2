...
import { ArrowRight, Database, FlaskConical, Gem, Shield, Swords, Trophy } from "lucide-react";
...
const stats = [
  { label: "Playable Shells", value: String(archiveCounts.namedShells), note: `${archiveCounts.shells} records including memories`, icon: Shield },
  { label: "Weapons + sidearms", value: `${archiveCounts.weapons} + ${archiveCounts.sidearms}`, note: "1.0 launch equipment", icon: Swords },
...
export default function WikiPage() {
  const schema = { "@context": "https://schema.org", "@type": "CollectionPage", name: "Mortal Shell II Wiki", url: `${siteConfig.url}/wiki/`, description: "A player-focused Mortal Shell II wiki for Shells, weapons, items, Tarstones, skills, Seals, status effects, and achievements. Numbers match the 1.0 launch version.", isPartOf: { "@type": "WebSite", name: siteConfig.name, url: siteConfig.url } };
  return (
    <>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroInner}>
          ...
          <div className={styles.heroCopy}><p><Link href="/wiki/weapons/">Weapons</Link>, <Link href="/wiki/shells/">Shells</Link>, <Link href="/wiki/items/">items</Link>, <Link href="/wiki/tarstones/">Tarstones</Link>, and <Link href="/wiki/skills/">skills</Link></p><h1>Mortal Shell II Wiki - Weapons, Shells, Items, and Skills</h1><div>Find the page you need for a <Link href="/wiki/weapons/">weapon</Link>, <Link href="/wiki/shells/">Shell</Link>, <Link href="/wiki/items/">item</Link>, <Link href="/wiki/tarstones/">Tarstone</Link>, <Link href="/wiki/skills/">skill</Link>, <Link href="/wiki/seals/">Seal</Link>, <Link href="/wiki/status-effects/">status effect</Link>, or <Link href="/wiki/achievements/">achievement</Link>. Wiki numbers match the 1.0 launch version.</div></div>
      ...
      <div className={styles.layout}>
        <div>
          <section className={styles.searchBanner} data-archive-navigator aria-label="Wiki archive navigator">
            <Database size={18} />
            <div><small>Archive navigator</small><p><Link href="/wiki/weapons/">Weapons</Link>, <Link href="/wiki/shells/">Shells</Link>, <Link href="/wiki/artifacts/">artifacts</Link>, and <Link href="/wiki/tarstones/">Tarstones</Link> use the 1.0 launch records.</p></div>
            <Link href="/wiki/weapons/">Browse weapons <ArrowRight size={14} /></Link>
          </section>
          ...
          <section className={styles.dataDigest} id="archive">
            ...
            <div className={styles.digestGrid}>
              {wikiColumns.map((column, index) => (
                <div key={index}>
                  <h3>{index === 0 ? "Equipment and Shells" : "Skills and reference"}</h3>
                  {column.map((entry) => {
