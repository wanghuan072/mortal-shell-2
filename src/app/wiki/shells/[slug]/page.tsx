...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return record ? getRecordMetadata(record.seo, `/wiki/shells/${record.id}/`) : {};
}
export default async function ShellDetailPage({ params }: PageProps) {
  const record = findArchiveRecord(shellArchive, (await params).slug);
  if (!record) notFound();
  return <ArchiveRecordPage basePath="/wiki/shells" parent={{ label: "Wiki", href: "/wiki/" }} record={record} related={shellArchive.filter((entry) => entry.id !== record.id)} title="Shells" />;
}
